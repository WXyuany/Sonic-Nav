import os, sys, math, threading, time, numpy as np
import numpy as np
import rclpy
from rclpy.node import Node
from nav_msgs.msg import Odometry
from sensor_msgs.msg import LaserScan
from geometry_msgs.msg import TransformStamped, Quaternion
from tf2_ros import TransformBroadcaster
from std_msgs.msg import Header

os.environ.setdefault("RMW_IMPLEMENTATION", "rmw_fastrtps_cpp")
os.environ.setdefault("ROS_LOCALHOST_ONLY", "1")
os.environ.setdefault("ROS_DOMAIN_ID", "42")

REPO = os.path.expanduser("~/GR00T-WholeBodyControl")
sys.path.insert(0, REPO)
sys.path.insert(0, os.path.join(REPO, "g1_ros2_nav"))

import mujoco
from gear_sonic.utils.mujoco_sim.scene_registry import resolve_scene
from g1_ros2_nav.lidar_sim import LidarSim


class StandaloneBridge(Node):
    def __init__(self):
        super().__init__("g1_bridge")
        scene_arg = os.environ.get("SONIC_SCENE", sys.argv[1] if len(sys.argv) > 1 else "default")
        xml_path = str(resolve_scene(scene_arg, repo_root=REPO).abs_path)
        self._model = mujoco.MjModel.from_xml_path(xml_path)
        self._data = mujoco.MjData(self._model)
        self._lidar = LidarSim(self._model, self._data, site_name="lidar")

        self._odom_pub = self.create_publisher(Odometry, "/odom", 10)
        self._scan_pub = self.create_publisher(LaserScan, "/scan", 10)
        self._tf_bc = TransformBroadcaster(self)

        self._pub_timer = self.create_timer(0.05, self._publish_all)
        self.get_logger().info("Standalone bridge started — /odom /scan /tf")

    def sync_state(self, qpos):
        self._data.qpos[:len(qpos)] = qpos
        mujoco.mj_forward(self._model, self._data)

    def _publish_all(self):
        try:
            qpos = np.load("/tmp/sonic_qpos.npy")
            self._data.qpos[:len(qpos)] = qpos
        except Exception:
            return
        mujoco.mj_forward(self._model, self._data)

        now = self.get_clock().now().to_msg()
        h = Header(stamp=now, frame_id="odom")

        pos = self._data.qpos[0:3].copy()
        quat = self._data.qpos[3:7].copy()

        t = TransformStamped()
        t.header = h
        t.child_frame_id = "base_link"
        t.transform.translation.x = float(pos[0])
        t.transform.translation.y = float(pos[1])
        t.transform.translation.z = float(pos[2])
        t.transform.rotation.w = float(quat[0])
        t.transform.rotation.x = float(quat[1])
        t.transform.rotation.y = float(quat[2])
        t.transform.rotation.z = float(quat[3])
        self._tf_bc.sendTransform(t)

        yaw = math.atan2(
            2.0 * (quat[0] * quat[3] + quat[1] * quat[2]),
            1.0 - 2.0 * (quat[2] * quat[2] + quat[3] * quat[3]),
        )
        odom = Odometry()
        odom.header = h
        odom.child_frame_id = "base_link"
        odom.pose.pose.position.x = float(pos[0])
        odom.pose.pose.position.y = float(pos[1])
        odom.pose.pose.position.z = float(pos[2])
        cy = math.cos(yaw * 0.5)
        sy = math.sin(yaw * 0.5)
        odom.pose.pose.orientation = Quaternion(w=cy, z=sy)
        self._odom_pub.publish(odom)

        self._lidar.step()
        d = self._lidar
        scan = LaserScan()
        scan.header = Header(stamp=now, frame_id="lidar_link")
        scan.angle_min = 0.0
        scan.angle_max = 2 * math.pi - d.angles[1]
        scan.angle_increment = float(d.angles[1] - d.angles[0])
        scan.range_min = float(d.min_range)
        scan.range_max = float(d.max_range)
        scan.ranges = [float(r) for r in d.ranges]
        self._scan_pub.publish(scan)


def main():
    rclpy.init()
    bridge = StandaloneBridge()
    try:
        rclpy.spin(bridge)
    except KeyboardInterrupt:
        pass
    bridge.destroy_node()
    rclpy.shutdown()


if __name__ == "__main__":
    main()
