...
ENV.setdefault("CARMA_MPPI_ROOT", os.path.expanduser("~/CARMA-MPPI-main"))
ENV.setdefault(
    "CARMA_MPPI_CHECKPOINT",
    os.path.join(ENV["CARMA_MPPI_ROOT"], "carma_train", "checkpoints", "carma_final.pth"),
)
procs = []
proc_names = {}
...
def check_processes():
    for p in list(procs):
        code = p.poll()
        if code is not None:
            name = proc_names.get(p.pid, f"pid {p.pid}")
...
add_proc(sim, "SIM")
print("[SIM] Starting..."); time.sleep(8); check_processes(); print("[SIM] Running")
...
time.sleep(2)
run_script("navigation/scene_map_server.py", "MAP", SCENE_XML)
time.sleep(1)
run_script("navigation/astar_global_planner.py", "GLOBAL_PLANNER")
time.sleep(1)
run_script("navigation/nav_control_adapter.py", "NAV_ADAPTER")
time.sleep(1)
run_script("perception/mid360_pub.py", "MID360", SCENE_XML)
time.sleep(2)
run_script("navigation/local_costmap_server.py", "LOCAL_COSTMAP")
time.sleep(1)
run_script("navigation/nav_metrics.py", "NAV_METRICS")
time.sleep(1)
...
run_script("navigation/mppi_nav.py", "MPPI")
