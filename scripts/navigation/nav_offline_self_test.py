...
from scripts.tools.randomize_scene import randomize_scene
...
def test_launch_scripts():
    launch_scripts = [
        SCRIPTS_DIR / "start.py",
        SCRIPTS_DIR / "start_dwa.py",
        SCRIPTS_DIR / "start_mppi.py",
        SCRIPTS_DIR / "start_box_demo.py",
        SCRIPTS_DIR / "start_ball_demo.py",
    ]
    for path in launch_scripts:
        text = path.read_text()
        matches = re.findall(r'run_script\("([^"]+)",\s*"([^"]+)"', text)
        _check(matches, f"{path.name} should launch child scripts through run_script")
        for rel_path, name in matches:
            child = SCRIPTS_DIR / rel_path
            _check(child.exists(), f"{path.name} launches missing script {rel_path} as {name}")
        _check(
            'run_script("tools/world_model_node.py", "WORLD_MODEL"' in text,
            f"{path.name} should launch the world-model node",
        )
        _check(
            'run_script("tools/world_model_executor.py", "WORLD_EXECUTOR"' in text,
            f"{path.name} should launch the world-model executor",
        )
        _check(
            'run_script("tools/world_model_recovery_coordinator.py", "WORLD_RECOVERY"' in text,
            f"{path.name} should launch the world-model recovery coordinator",
        )
        node_index = text.find('run_script("tools/world_model_node.py", "WORLD_MODEL"')
        recovery_index = text.find('run_script("tools/world_model_recovery_coordinator.py", "WORLD_RECOVERY"')
        executor_index = text.find('run_script("tools/world_model_executor.py", "WORLD_EXECUTOR"')
        _check(
            node_index < recovery_index < executor_index,
            f"{path.name} should launch world-model node, recovery coordinator, then executor",
        )
